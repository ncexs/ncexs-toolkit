...
### ⚡ 빠른 수정 및 진단 (Quick Fix & Diagnostics)
* **원클릭 유지 관리:** 단일 메뉴 트리거를 통해 정크 정리, 네트워크 복구 및 RAM 최적화를 결합한 자동화된 시퀀스를 실행합니다.
* **실시간 진행률 추적:** 터미널에서 직접 직관적인 백분율 표시줄을 통해 실시간으로 작업 실행을 모니터링합니다.
* **시스템 상태 검사:** 맞춤형 UI 렌더링으로 기본 `SFC` 및 `DISM` 도구를 실행하여 진행 상황을 명확하게 파악할 수 있습니다.

### 🧹 스토리지 및 정리 (Storage & Clean Up)
* **Compact OS 압축:** 시스템 성능이나 기능을 손상시키지 않고 시스템 파일을 안전하게 압축하여 **2GB – 5GB**의 저장 공간을 확보합니다.
* **향상된 정크 클리너:** 임시 시스템 폴더, Prefetch 파일, GPU 셰이더 캐시 및 여러 활성 브라우저 프로필(Chrome, Edge, Brave, WhatsApp Web)을 심층 정리합니다.
* **스마트 앱 제거 프로그램:** 활성 시스템 레지스트리를 통한 빠른 퍼지 검색으로 제거 스크립트를 즉시 찾고 실행합니다.
* **휴지통 및 디스크 정리:** 삭제된 파일을 안전하게 영구 삭제하거나 Windows 기본 디스크 정리 인터페이스를 즉시 실행합니다.
...
### 🌐 네트워크 및 관리 도구 (Network & Admin Tools)
* **네트워크 복구 도구:** 손상된 인터넷 연결을 복구하기 위해 5가지 필수 복구 도구(TCP/IP 재설정, Winsock 재설정, DNS 플러시, IP 해제/갱신)를 자동화합니다.
* **DNS 프로필 전환기:** Cloudflare(1.1.1.1), Google DNS(8.8.8.8), Quad9, AdGuard(광고 차단) 또는 표준 DHCP 간에 빠르게 전환합니다.
...
*시스템 유지 관리를 실행하기 전에 항상 중요한 파일을 백업하세요.* ⚠️
...
[![GitHub Star](https://img.shields.io/badge/GitHub-프로젝트%20별%20누르기-blue?style=flat&logo=github)](https://github.com/ncexs/toolkit)
